...
train_data <- read.csv(file = "D:/IDA/HW6/Train.csv")
test_data <- read.csv(file = "D:/IDA/HW6/Test.csv")

# glimpse(train_data)
# summary(train_data)
skim(train_data)
num<- train_data %>% select(where(is.numeric))
num <- select(train_data,is.numeric)
matrix<-cor(num)
heatmap(matrix)
...
cv_mars <- train(
  x = subset(final_train, select = -sum_revenue),
  y = final_train$sum_revenue,
  method = "earth",
  metric = "RMSE",
  trControl = trainControl(method = "cv", number = 10),
  tuneGrid = hyper_grid
)
accuracy(mars,plotit = TRUE)
rmse(log(final_train$sum_revenue+1),cv_mars )
mars
plot(mars)
...
modelsvm = svm(log(sum_revenue+1)~log(max_visitNumber +1) + browser_first + first_country +
                 first_networkDomain +log(max_bounces+1)+log(numofVisits +1)+
                 log(1+avg_pageViews) + log(1+sum_pageviews) + log(1+max_pageviews) +
                 log(sum_bounces+1)+ first_source , data= final_train)

plot(modelsvm)
cor(final_test, svm_predict) ^ 2
modelsvm
...
#RMSE : 0.7131
predRevenue <- predict(svm_predict,final_test)
predRevenue <- replace(predRevenue,which(predRevenue<0),0)
svm_result <- tibble(custId=final_test$custId, predRevenue)
write.csv(svm_result, 'submission_svm.csv', row.names = F)
